...
class ImageManager extends Manager
{
    ...
    public function url($src, $width = null, $height = null, $options = array())
    {
        ...
        $parts = pathinfo($src);
        $host = isset($options['host']) ? $options['host']:app()['config']['image.host'];
        $dir = trim($parts['dirname'], '/');

        $path = array();
        $path[] = rtrim($host, '/');

        if ($prefix = app()['config']->get('image.write_path')) {
            $path[] = trim($prefix, '/');
        }
    ...
    public function serve($path, $config = array())
    {
        ...
        $quality = array_get($config, 'quality', app()['config']['image.quality']);
        ...
        $config = array_merge(array(
            ...
            'write_path' => app()['config']['image.write_path']
        ), $config);
    ...
    public function proxy($path, $config = array())
    {
        //Merge config with defaults
        $config = array_merge(array(
            'tmp_path' => app()['config']['image.proxy_tmp_path'],
            'filesystem' => app()['config']['image.proxy_filesystem'],
            'cache' => app()['config']['image.proxy_cache'],
            'cache_expiration' => app()['config']['image.proxy_cache_expiration'],
            'write_image' => app()['config']['image.proxy_write_image'],
            'cache_filesystem' => app()['config']['image.proxy_cache_filesystem']
        ), $config);
    ...
    public function parse($path, $config = array())
    {
        ...
        $config = array_merge(array(
            ...
            'url_parameter' => null,
            'url_parameter_separator' => app()['config']['image.url_parameter_separator']
        ), $config);
    ...
    public function getRealPath($path)
    {
        ...
        //Get directories
        $dirs = app()['config']['image.src_dirs'];
        if (app()['config']['image.write_path']) {
            $dirs[] = app()['config']['image.write_path'];
        }
    ...
    {
        return app()['config']['image.driver'];
    }
